...
| title | Object | | 是 | 提示的内容 | | | icon | Object | | 是 | 图标 | | | image | Object | | 是 | 自定义图标的本地路径，image 的优先级高于 icon | 1.1.0 | | duration | Object | | 是 | 提示的延迟时间 | | | success | function | | 否 | 接口调用成功的回调函数 | | | fail | function | | 否 | 接口调用失败的回调函数 | | | complete | function | | 否 | 接口调用结束的回调函数（调用成功、失败都会执行） | |
