...
**File 对象结构如下:** | 字段 | 类型 | 说明 | |------|--------|-----------------------| | path | String | 本地文件路径 | | size | Number | 本地文件大小，单位：B |
...
保存图片到系统相册，需要用户授权（scope.writePhotosAlbum），详见 [用户授权](authorize.html#wxauthorizeobject)
...
<td>图片文件路径，可以是临时文件路径也可以是永久文件路径</td>
