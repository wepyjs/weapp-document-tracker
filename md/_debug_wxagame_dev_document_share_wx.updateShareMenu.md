...
| withShareTicket | boolean | | 是 | 是否使用带 shareTicket 的转发<a href="">详情</a> | | | success | function | | 否 | 接口调用成功的回调函数 | | | fail | function | | 否 | 接口调用失败的回调函数 | | | complete | function | | 否 | 接口调用结束的回调函数（调用成功、失败都会执行） | |
...
[](wx.shareAppMessage.html)[](../performance/wx.getPerformance.html)</div>
