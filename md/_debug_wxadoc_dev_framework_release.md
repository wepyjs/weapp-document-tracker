...
1.  `A` 新增小游戏 API `wx.navigateToMiniProgram` [详情](https://developers.weixin.qq.com/minigame/dev/document/open-api/miniprogram/wx.navigateToMiniProgram.html)
2.  `F` 修复 API `wx.scanCode` 在安卓平台返回 `scanType` 错误问题
...
14.  `A` 新增 配置 `window.backgroundColorTop` `window.backgroundColorBottom` 可分别设置窗口顶部和底部的背景颜色 [详情](config.html)
...
28.  `U` 更新 组件 新增 `bindtouchforcechange` 事件 [详情](view/wxml/event.html)
29.  `U` 更新 自定义组件 样式前缀化支持类名传递 [详情](custom-component/wxml-wxss.html)
...
1.  `A` 新增 配置 `window.navigationStyle` 支持全屏显示小程序 [详情](config.html)
...
20.  `U` 更新 框架 `referrerInfo.appId` onLaunch 及 onShow 时部分跳转小程序场景增加返回 referrerInfo.appId [详情](app-service/app.html)
...
4.  `A` 新增 框架 触摸类事件增加 capture 阶段 [详情](view/wxml/event.html)
5.  `A` 新增 框架 长按事件 `longpress` [详情](view/wxml/event.html)
6.  `A` 新增 框架 page.json 支持设置 onReachBottom 距离 [详情](config.html)
...
52.  `U` 更新 框架 `setData` 支持异步回调 [详情](app-service/page.html#pageprototypesetdata)
...
59.  `A` 新增 开发体验 客户端增加性能面板 [详情](performance.md)
...
26.  `A` 新增 框架能力 可通过 `App.onLaunch`、`App.onShow` 获取进入小程序的场景值 [详情](app-service/app.html#场景值)
...
7.  `A` 新增 框架方法 `App.onError` [详情](app-service/app.html)
8.  `A` 新增 页面配置 `disabelScroll` [详情](config.html#pagejson)
...
16.  `A` 新增 `getCurrentPages()` 方法， 获取页面栈 [详情](app-service/page.html#getcurrentpages)
17.  `A` 新增 `Page.onReachBottom()` 方法，监听页面到达底部 [详情](app-service/page.html)
18.  `A` 新增 事件对象增加 changedTouches 列表，反应手指触摸位置的变化 [详情](view/wxml/event.html)
...
27.  `A` 新增 WXML `wx:key` 支持，提升列表渲染时性能 [详情](view/wxml/list.html#wxkey)
28.  `A` 新增 WXML 关键字、数据路径计算 [详情](view/wxml/data.html)
29.  `U` 新增 app.json `tabBar` 属性 `position` 用于指定显示位置 [详情](config.html#tabbar)
...
41.  `D` 即将移除 `App.prototype.getCurrentPage`，可使用更功能更强大的 `getCurrentPages` 替代 [详情](app-service/app.html#appprototypegetcurrentpage)
...
[](performance/tools.html)[](../component/)</div>
