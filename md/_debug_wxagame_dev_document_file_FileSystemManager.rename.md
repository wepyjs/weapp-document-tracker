...
| oldPath | string | | 是 | 源文件路径，可以是普通文件或目录 | | | newPath | string | | 是 | 新文件路径 | | | success | function | | 否 | 接口调用成功的回调函数 | | | fail | function | | 否 | 接口调用失败的回调函数 | | | complete | function | | 否 | 接口调用结束的回调函数（调用成功、失败都会执行） | |
