...
从小程序基础库版本 [2.2.1](../framework/compatibility.html "基础库 2.2.1 开始支持，低版本需做兼容处理。") 开始，小程序支持使用 npm 安装第三方包。 开发时需要使用 [beta 版开发者工具](beta.html)。
...
1.  在小程序中执行命令安装 npm 包：
    ...
    > PS：此处请务必使用`--production`选项，可以减少安装一些业务无关的 npm 包，从而减少整个小程序包的大小。 PS：原则上并没有强制要求 node_modules 必须在小程序根目录中，也可以存在于小程序的各个子目录中。

2.  点击开发者工具中的菜单栏：工具 --> 构建 npm  
    ![construction](https://developers.weixin.qq.com/miniprogram/dev/image/framework/construction.png)
3.  勾选“使用 npm 模块”选项：  
    ![use npm](https://developers.weixin.qq.com/miniprogram/dev/image/framework/use_npm.png)
4.  构建完成后即可使用 npm 包。 js 中引入 npm 包：
...
1.  小程序 npm 包要求根目录下必须有 miniprogram_dist 目录，并且该目录下必须有 index.js 文件。miniprogram_dist/index.js 即会作为小程序 npm 包的入口文件。如果是小程序 npm 包提供的是自定义组件，那么 miniprogram_dist 目录下至少有 index.js、index.wxml、index.wxss、index.json 四个文件，即一个完整的自定义组件的文件构成。
2.  小程序 npm 包里只有 miniprogram_dist 目录会被算入小程序包的占用空间，上传小程序代码时也只会上传该目录的代码。如果小程序 npm 包有一些测试、构建相关的代码请放到 miniprogram_dist 目录外。另外也可以使用`.npmignore`文件来避免将一些非业务代码文件发布到 npm 中。
...
2.  构建打包分为两种：小程序 npm 包会直接拷贝 miniprogram_dist 目录下的所有文件到 miniprogram_npm 中；其他 npm 包则会从入口 js 文件开始走一遍依赖分析和打包过程（类似 webpack）。
...
[](multiaccount.html)[](audits.html)</div>
